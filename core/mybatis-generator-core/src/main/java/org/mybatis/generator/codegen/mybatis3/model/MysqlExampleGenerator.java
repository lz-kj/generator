...
public class MysqlExampleGenerator extends AbstractJavaGenerator {

    public MysqlExampleGenerator(String project) {
        super(project);
    }
    ...
    private InnerClass getCriteriaInnerClass() {
        ...
        Method baseQuery = new Method("baseQuery"); //$NON-NLS-1$
        baseQuery.setVisibility(JavaVisibility.PUBLIC);
        baseQuery.setConstructor(false);
        baseQuery.setReturnType(FullyQualifiedJavaType.getCriteriaInstance());
        baseQuery.addParameter(new Parameter(FullyQualifiedJavaType.getNewMapInstance(), "params"));
        baseQuery.addBodyLine("BaseGenerateUtil.andEqualToQuery(params,this);");
        baseQuery.addBodyLine("return this;"); //$NON-NLS-1$
        answer.addMethod(baseQuery);
    ...
    private InnerClass getGeneratedCriteriaInnerClass(
        ...
        //继承接口
        FullyQualifiedJavaType clsBaseMapper = new FullyQualifiedJavaType("IGenerate<Criteria>");
        answer.addSuperInterface(clsBaseMapper);
        String baseGenerateImport = "cn.com.lz.generator.mysql.plugins.base.model.IGenerate";
        FullyQualifiedJavaType impBaseMapper = new FullyQualifiedJavaType(baseGenerateImport);
        topLevelClass.addImportedType(impBaseMapper);
        String utilImport = "cn.com.lz.generator.mysql.util.BaseGenerateUtil";
        FullyQualifiedJavaType utilImportType = new FullyQualifiedJavaType(utilImport);
        topLevelClass.addImportedType(utilImportType);
